      ...
      <div style={{ maxWidth:1600, margin:"0 auto" }}>
        ...
        <div style={{ marginBottom:24 }}>
          ...
          <p style={{ fontSize:13, color:"#4A6B8A", marginTop:6, marginBottom:0 }}>
            Comprehensive KPI tracking across 9 pricing process stages · 24 metrics · 7 product lines
          </p>
        </div>
          ...
          {PRODUCT_LINES.map(pl => {
            ...
            return (
              <button
                ...
                onMouseEnter={(e) => {
                  if (!isActive) {
                    e.currentTarget.style.background = "#111C2A";
                ...
                onMouseLeave={(e) => {
                  if (!isActive) {
                    ...
                    e.currentTarget.style.borderColor = "#1E3554";
                  }
                }}
              >
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize:18, fontWeight:700, margin:0, color:"white" }}>
              Value Waterfall — {PRODUCT_LINES.find(p => p.id === selectedLine)?.label}
            </h2>
            <div style={{
              background:"#0D1520",
              border:"1px solid #1E3554",
              ...
              Net Impact: {netTotal > 0 ? "+" : ""}{netTotal}%
            </div>
          </div>
              ...
              {stages.map((s, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    <text
                      x={x + barWidth / 2}
                      y={isPos ? barY - 8 : barY + barHeight + 16}
                      fill={isHovered ? "white" : "#C8D8EC"}
                      fontSize={11}
                      fontWeight={700}
                      textAnchor="middle"
                      fontFamily="monospace"
                    ...
                    <text
                      x={x + barWidth / 2}
                      y={chartHeight - 8}
                      fill={isHovered ? "#C8D8EC" : "#4A6B8A"}
                      fontSize={9}
                      textAnchor="middle"
                      style={{ whiteSpace:"pre" }}
                    >
                      {s.label.split("\n").map((line, li) => (
                        <tspan key={li} x={x + barWidth / 2} dy={li === 0 ? 0 : 11}>
                          {line}
                        </tspan>
                      ))}
                    </text>
